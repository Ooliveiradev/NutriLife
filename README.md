...
Arquivos principais:
- `lista.html` (p�gina SPA)
- `css/pages/shopping-list.css` (estilos + @media print)
- `js/shopping/*` (m�dulos: firebase, app, recipes, aggregation, ingredients, exporters, ui)
...
- O m�dulo `js/shopping/firebase.js` usa Firebase v9 modular (CDN) e habilita persist�ncia offline quando poss�vel.

Como rodar localmente:
- Sirva o diret�rio (ex.: `npx serve`).
- Abra `http://localhost:3000/lista.html`.

Regras de seguran�a:
- `users/{uid}`: somente o pr�prio usu�rio l�/escreve.
- `users/{uid}/lists/{listId}`: somente `uid` acessa; valida campos b�sicos.
- `recipes/{id}`: leitura p�blica; escrita apenas se `request.auth.token.admin == true`.

Convers�es de unidades:
- Aproxima��es culin�rias documentadas em `js/shopping/aggregation.js`.
- Para s�lidos, base em `g`; l�quidos em `ml`.
